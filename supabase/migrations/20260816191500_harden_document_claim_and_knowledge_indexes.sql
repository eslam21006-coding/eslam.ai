-- Applied migrations are immutable. Harden the document extraction claim forward-only and
-- add indexes that match the current global Knowledge Library access paths.
...
begin
  ...
  -- document_upload_id is unique. Lock that row regardless of owner so an inconsistent
  -- historical row cannot be missed and followed by a unique-constraint failure on INSERT.
  select * into v_extraction
  from public.document_teaching_extractions
  where document_upload_id = p_document_id
  for update;

  if found and v_extraction.created_by <> p_created_by then
    return query select null::uuid, 'not_found'::text, 0, null::uuid;
    return;
  end if;
...
create index if not exists knowledge_sources_expired_claim_idx
  on public.knowledge_sources (index_lease_expires_at, id)
  where status = 'indexing'
    and index_claim_token is not null;
