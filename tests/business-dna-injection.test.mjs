...
test("Business DNA serializer and request wiring preserve the intended source contracts", () => {
  ...
  assert.match(
    request,
    /buildInstructions\([\s\S]*businessDnaContext,[\s\S]*eslamBrainContext,[\s\S]*Boolean\(knowledgeVectorStoreId\)/,
  );
  assert.match(
    request,
    /buildBasicEslamResponseRequest\([\s\S]*businessDnaContext,[\s\S]*eslamBrainContext,[\s\S]*knowledgeVectorStoreId/,
  );
...
test("blocking and streaming response paths pass the same owner Business DNA context", () => {
  ...
  assert.match(
    actions,
    /generateBasicEslamReply\([\s\S]*messages,[\s\S]*businessDnaContext,[\s\S]*eslamBrainContext,[\s\S]*knowledgeVectorStoreId/,
  );

  assert.match(route, /const userId = await getAuthenticatedUserId\(\)/);
  assert.match(route, /loadBusinessDnaModelContext\(userId\)/);
  assert.match(route, /streamBasicEslamReply\([\s\S]*businessDnaContext,[\s\S]*eslamBrainContext,[\s\S]*knowledgeVectorStoreId/);
